use crate::{vm::{VM, VMRunner}, input::Key};

use crossterm::event::{KeyCode, KeyEventKind, Event, KeyEvent};
use tui::{layout::{Rect, Layout, Direction, Constraint}, widgets::StatefulWidget, style::Style};
...
impl Debugger {
    ...
    pub fn handle_input_event(&mut self, event: Event, vm_runner: &mut VMRunner) -> bool {
        // if this state changes then we want to rerender
        ...
        rerender = rerender || !(shell.input.len() == input_len && shell.cursor_position == cursor_position && shell.history_index == history_index);
        rerender
    }
    ...
    pub fn step(&mut self, vm: &VM) -> bool {
        ...
        for &watchpoint in self.watchpoints.iter() {
            let (old_val, new_val) = match watchpoint {
                ...
                Watchpoint::Register(vx) => (self.watch_state.registers[vx as usize] as u16, interp.registers[vx as usize] as u16)
            };

            if old_val != new_val {
                self.events.push(DebugEvent::WatchpointChange(watchpoint, old_val, new_val));
            }
        }
        if self.breakpoints.contains(&interp.pc) {
            self.events.push(DebugEvent::BreakpointReached(interp.pc));
        }
...
impl<'a> DebuggerWidget<'a> {
    ...
    pub fn cursor_position(&self, area: Rect, state: &mut DebuggerWidgetState) -> Option<(u16, u16)> {
    ...
    fn areas(&self, area: Rect) -> (Rect, Rect, Rect) {
        ...
        let (main, logger) = if self.logger_enabled {
            let rects = Layout::default()
...
impl<'a> From<&'a Debugger> for DebuggerWidget<'a> {
    fn from(dbg: &'a Debugger) -> Self {
        DebuggerWidget { dbg, logger_enabled: false }
    }
}
...
impl<'a> ShellWidget<'_> {
    ...
    fn cursor_position(&self, area: Rect, state: &mut ShellWidgetState) -> Option<(u16, u16)> {
        if area.area() == 0 || !self.shell.input_enabled {
            ...
            let (cmd_x, cmd_y, _, cmd_prefix_width, input_area_width) = self.compute_draw_params(area);
            
            if input_area_width > 0 {
                if self.shell.cursor_position < state.input_offset {
                    state.input_offset = self.shell.cursor_position
                } else if self.shell.cursor_position >= state.input_offset + input_area_width as usize {
                    state.input_offset = self.shell.cursor_position - (input_area_width as usize - 1)
                }

                if state.input_offset + (input_area_width - 1) as usize > self.shell.input.len() {
                    state.input_offset = self.shell.input.len().saturating_sub(input_area_width as usize);
                }

                let cursor_x = cmd_x + cmd_prefix_width as u16 + (self.shell.cursor_position - state.input_offset) as u16;
                let cursor_y = cmd_y;
...
impl<'a> StatefulWidget for ShellWidget<'a> {
    ...
    fn render(self, area: Rect, buf: &mut tui::buffer::Buffer, state: &mut Self::State) { 
        ...
        if shell.input_enabled { 
            let (cmd_x, cmd_y, cmd_width, cmd_prefix_width, input_area_width) = self.compute_draw_params(area);
